...
### 3. Edit the created template to create the translation and compile **.mo** file from it to see your work ###
  ...
  * Open the file ``Flowblade.po`` in a text editor. Translations are given by writing the the translations inside quotes on lines staring with text ``msgstr``. To translate the menu item *Open...* you would need to fill the ``msgstr`` in the example below:
```bash
#: useraction.py:489
msgid "Open.."
msgstr ""
