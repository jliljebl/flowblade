...
def show_blender_container_clip_program_editor(callback, container_data):
    program_info_json = container_data.data_slots["project_edit_info"]
...
# ----------------------------------------------------- editors
def get_editor(editor_type, label_text, value):
    if editor_type == SIMPLE_EDITOR_TEXT:
        return TextEditor(label_text, value)
...
class TextEditor(AbstractSimpleEditor):
    ...
    def __init__(self, label_text, value):
        AbstractSimpleEditor.__init__(self)
